...
def test_simulate_pipeline_with_extends_and_variable_precedence():
        ...
        "Specific Job VCS": {
            "extends": ["Specific Job"],
             "rules": [
                ...
                {"if": '$CI_PIPELINE_SOURCE == "web" && $RUN_SPECIFIC_VCS == "1"'}
            ],
           "variables": {
                "TOOL": "vcs_tool" # Overrides Specific Job's TOOL
            }
        }
    }
    ...
    global_variables = {
        ...
        "RUN_SPECIFIC_VCS": "1", # To make Specific Job VCS run
        "RESULTS_DIR": "/path/to/results" # Example path
        # Note: BASE_FLAGS is inherited from .Base Job
        # Note: RUN_FLAGS is overridden by Specific Job, then inherited by Specific Job VCS
    }
    ...
    assert "Specific Job VCS" in jobs
    ...
    # --- Verify Specific Job VCS ---
    # Verify the VCS job's expanded script content
    # TOOL should be 'vcs_tool' (from job)
    ...
    expected_script_vcs = [
        "echo \"Build specific_module with vcs_tool. Base flags are -b base. Run flags are -r specific.\"",
        "compile specific_module -tool vcs_tool -b base -flags \" -r specific\""
    ]
    assert jobs["Specific Job VCS"]["script"] == expected_script_vcs
    assert jobs["Specific Job VCS"]["variables"]["TOOL"] == "vcs_tool" # Check variable value itself

    # Verify the expanded artifacts path for Specific Job VCS
    assert jobs["Specific Job VCS"]["artifacts"]["paths"] == ["/path/to/results/"]
