...
"""
    ...
    monkeypatch.setattr("sys.argv", ["cli.py", "simulate", ci_file, sim_file, "Nightly-Avispado", "--output", output_file])
    try:
        main()
        # Check if the success message is printed to stdout
        captured = capsys.readouterr().out
        ...
        with open(output_file, 'r') as f:
            ...
            assert "RUN_NIGHTLY_TECORIGIN: '1'" in content or 'RUN_NIGHTLY_TECORIGIN: "1"' in content
    finally:
        os.remove(ci_file)
        os.remove(sim_file)
        # Clean up the output file
